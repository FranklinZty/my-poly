//! A sparse multivariate polynomial represented in group coefficient form.
use crate::{
    multivariate::{SparseTerm, Term},
    DenseMVGroupPolynomial, GroupPolynomial,
};
...
pub struct SparseGroupPolynomial<G: CurveGroup, T: Term> {
...
impl<G: CurveGroup, T: Term> SparseGroupPolynomial<G, T> {
    fn remove_zeros(&mut self) {
        self.terms.retain(|(c, _)| !c.is_zero());
    }
}

impl<G: CurveGroup> GroupPolynomial<G> for SparseGroupPolynomial<G, SparseTerm> {
    ...
    /// ```
    /// use ark_poly::{
    ///     polynomial::multivariate::{SparseGroupPolynomial, SparseTerm},
    ///     DenseMVGroupPolynomial, GroupPolynomial,
    /// };
    ...
    /// let poly: SparseGroupPolynomial<G1Projective, SparseTerm> = SparseGroupPolynomial::rand(7, 2, rng);
    ...
    /// use ark_poly::{
    ///     polynomial::multivariate::{SparseGroupPolynomial, SparseTerm, Term},
    ///     DenseMVGroupPolynomial, GroupPolynomial,
    /// };
    /// use ark_std::test_rng;
    /// use ark_test_curves::bls12_381::{Fr, G1Projective};
    ///
    /// let rng = &mut test_rng();
    /// let poly = SparseGroupPolynomial::rand(4, 3, rng);
    /// let random_point = vec![Fr::rand(rng); 3];
    /// // The result will be a single element in the field.
    /// let result: G1Projective = poly.evaluate(&random_point).into();
    /// ```

    fn evaluate(&self, point: &Vec<G::ScalarField>) -> G {
        assert!(point.len() >= self.num_vars, "Invalid evaluation domain");
        if self.is_zero() {
            return G::zero();
        }
        cfg_into_iter!(&self.terms)
            .map(|(coeff, term)| coeff.mul(term.evaluate(point)))
            .sum()
    }
}

impl<G: CurveGroup> DenseMVGroupPolynomial<G> for SparseGroupPolynomial<G, SparseTerm> {
    ...
    fn rand<R: Rng>(d: usize, l: usize, rng: &mut R) -> Self {
        let mut random_terms = vec![(G::rand(rng), SparseTerm::new(vec![]))];
        for var in 0..l {
            for deg in 1..=d {
                random_terms.push((G::rand(rng), SparseTerm::new(vec![(var, deg)])));
            }
        }
    ...
    /// let poly = SparseGroupPolynomial::from_coefficients_vec(
    ///     3,
    ///     vec![
    ///         (G1Projective::from(2), SparseTerm::new(vec![(0, 3)])),
    ///         (G1Projective::from(1), SparseTerm::new(vec![(0, 1), (2, 1)])),
    ///         (G1Projective::from(1), SparseTerm::new(vec![(1, 1), (2, 1)])),
    ///         (G1Projective::from(5), SparseTerm::new(vec![])),
    ///     ],
    /// );
    /// ```
    fn from_coefficients_vec(num_vars: usize, mut terms: Vec<(G, SparseTerm)>) -> Self {
        // Ensure that terms are in ascending order.
        terms.sort_by(|(_, t1), (_, t2)| t1.cmp(t2));
        // If any terms are duplicated, add them together
        let mut terms_dedup: Vec<(G, SparseTerm)> = Vec::new();
        for term in terms {
    ...
    fn terms(&self) -> &[(G, Self::Term)] {
        self.terms.as_slice()
    }
}

impl<G: CurveGroup, T: Term> Add for SparseGroupPolynomial<G, T> {
    type Output = SparseGroupPolynomial<G, T>;

    fn add(self, other: SparseGroupPolynomial<G, T>) -> Self {
        &self + &other
    }
}

impl<'a, 'b, G: CurveGroup, T: Term> Add<&'a SparseGroupPolynomial<G, T>> for &'b SparseGroupPolynomial<G, T> {
    type Output = SparseGroupPolynomial<G, T>;

    fn add(self, other: &'a SparseGroupPolynomial<G, T>) -> SparseGroupPolynomial<G, T> {
...
impl<'a, G: CurveGroup, T: Term> AddAssign<&'a SparseGroupPolynomial<G, T>> for SparseGroupPolynomial<G, T> {
    fn add_assign(&mut self, other: &'a SparseGroupPolynomial<G, T>) {
        *self = &*self + other;
    }
}

impl<'a, G: CurveGroup, T: Term> AddAssign<(G::ScalarField, &'a SparseGroupPolynomial<G, T>)> for SparseGroupPolynomial<G, T> {
    fn add_assign(&mut self, (f, other): (G::ScalarField, &'a SparseGroupPolynomial<G, T>)) {
        let other = Self {
            num_vars: other.num_vars,
            terms: other
                .terms
                .iter()
                .map(|(coeff, term)| (coeff.mul(f), term.clone()))
                .collect(),
        };
...
impl<G: CurveGroup, T: Term> Neg for SparseGroupPolynomial<G, T> {
    type Output = SparseGroupPolynomial<G, T>;

    #[inline]
    fn neg(mut self) -> SparseGroupPolynomial<G, T> {
...
impl<'a, 'b, G: CurveGroup, T: Term> Sub<&'a SparseGroupPolynomial<G, T>> for &'b SparseGroupPolynomial<G, T> {
    type Output = SparseGroupPolynomial<G, T>;

    #[inline]
    fn sub(self, other: &'a SparseGroupPolynomial<G, T>) -> SparseGroupPolynomial<G, T> {
        let neg_other = other.clone().neg();
        self + &neg_other
    }
}

impl<'a, G: CurveGroup, T: Term> SubAssign<&'a SparseGroupPolynomial<G, T>> for SparseGroupPolynomial<G, T> {
    #[inline]
    fn sub_assign(&mut self, other: &'a SparseGroupPolynomial<G, T>) {
        *self = &*self - other;
    }
}

impl<G: CurveGroup, T: Term> fmt::Debug for SparseGroupPolynomial<G, T> {
...
impl<G: CurveGroup, T: Term> Zero for SparseGroupPolynomial<G, T> {
...
mod tests {
    ...
    fn rand_poly<R: Rng>(l: usize, d: usize, rng: &mut R) -> SparseGroupPolynomial<G1Projective, SparseTerm> {
        ...
        random_terms.push((G1Projective::rand(rng), SparseTerm::new(vec![])));
        for _ in 1..num_terms {
            let term = (0..l)
                ...
                .flatten()
                .collect();
            let coeff = G1Projective::rand(rng);
            random_terms.push((coeff, SparseTerm::new(term)));
        }
        SparseGroupPolynomial::from_coefficients_slice(l, &random_terms)
    }

    // /// Perform a naive n^2 multiplication of `self` by `other`.
    // fn naive_mul(
    //     cur: &SparseGroupPolynomial<G1Projective, SparseTerm>,
    //     other: &SparseGroupPolynomial<G1Projective, SparseTerm>,
    // ) -> SparseGroupPolynomial<G1Projective, SparseTerm> {
    //     if cur.is_zero() || other.is_zero() {
    //         SparseGroupPolynomial::zero()
    //     } else {
    ...
    //             }
    //         }
    //         SparseGroupPolynomial::from_coefficients_slice(cur.num_vars, result_terms.as_slice())
    //     }
    // }

    #[test]
    fn test_arith_group_polynomials() {

        let now = Instant::now();
        let rng = &mut test_rng();
        let num_vars = 3;
        let max_degree = 1;
        let p1: SparseGroupPolynomial<ark_ec::short_weierstrass::Projective<ark_test_curves::bls12_381::g1::Config>, SparseTerm> = rand_poly(num_vars, max_degree, rng);
        let p2: SparseGroupPolynomial<ark_ec::short_weierstrass::Projective<ark_test_curves::bls12_381::g1::Config>, SparseTerm> = rand_poly(num_vars, max_degree, rng);
        let elapsed_time = now.elapsed();
        println!("Generate Random polynomial: {:?}", elapsed_time);

        let now = Instant::now();
        let _ = &p1 + &p2;
        let elapsed_time = now.elapsed();
        println!("Add two polynomials: {:?}", elapsed_time);
        ...
        let mut point = Vec::with_capacity(num_vars);
            ...
            for (coeff, term) in res.terms.iter() {
                let mut summand = *coeff;
                for var in term.iter() {
                    let eval = point.get(var.0).unwrap();
                    summand = summand.mul(eval.pow(&[var.1 as u64]));
                }
                total += summand;
            }
        let eval = res.evaluate(&point);
        assert_eq!(eval, total);
        let elapsed_time = now.elapsed();
        println!("Evaluate polynomial: {:?}", elapsed_time);
    }

    // #[test]
    // fn sub_group_polynomials() {
    //     let rng = &mut test_rng();
    //     let max_degree = 1;
    ...
    // #[test]
    // fn add_and_evaluate_group_polynomials() {
    //     let rng = &mut test_rng();
    //     let max_degree = 1;
    //     for a_var_count in 1..20 {
    //         for b_var_count in 1..20 {
    ...
    //     let a: SparseGroupPolynomial<_, SparseTerm> = SparseGroupPolynomial::from_coefficients_slice(
    //         4,
    //         &[
    ...
    //         ],
    //     );
    //     let b = SparseGroupPolynomial::from_coefficients_slice(
    //         4,
    //         &[
    ...
    //     let expected = SparseGroupPolynomial::from_coefficients_slice(
    //         4,
    //         &[
