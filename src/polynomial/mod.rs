...
/// Describes the common interface for group univariate and multivariate polynomials
pub trait GroupPolynomial<G: CurveGroup>:
    Sized
    + Clone
    + Debug
    + Hash
...
/// Describes the interface for univariate polynomials
pub trait DenseUVGroupPolynomial<G: CurveGroup>: GroupPolynomial<G, Point = G::ScalarField> {
    /// Constructs a new polynomial from a list of coefficients.
    fn from_coefficients_slice(coeffs: &[G]) -> Self;

    /// Constructs a new polynomial from a list of coefficients.
    fn from_coefficients_vec(coeffs: Vec<G>) -> Self;
...
/// Describes the interface for multivariate polynomials
pub trait DenseMVGroupPolynomial<G: CurveGroup>: GroupPolynomial<G> {
    /// The type of the terms of `self`
    type Term: multivariate::Term;

    /// Constructs a new polynomial from a list of tuples of the form `(coeff, Self::Term)`
    fn from_coefficients_slice(num_vars: usize, terms: &[(G, Self::Term)]) -> Self {
        Self::from_coefficients_vec(num_vars, terms.to_vec())
    }

    /// Constructs a new polynomial from a list of tuples of the form `(coeff, Self::Term)`
    fn from_coefficients_vec(num_vars: usize, terms: Vec<(G, Self::Term)>) -> Self;
